...
- **Email 標題修改**：將 Email 模板中的大標題從「Gooaye AI」改回「財經 AI 秘書」，以符合寄件者名稱。
- **移除私人信箱**：從開發日誌 (`CHANGELOG.md`) 中移除了殘留的私人信箱資訊，保護隱私。

## [Release 3.3.2] - 2026-03-20

### 💄 介面優化 (UI/UX Improvements)
- **Email 閱讀體驗升級**：針對長輩與大螢幕使用者優化信件排版。將信件最大寬度從 `650px` 放寬至 `850px`，並將整體基礎字體大小放大約 20%（至 `19px`），同時增加行距（`1.6` -> `1.8`），讓閱讀長篇財經摘要時眼睛更舒適、不吃力。
...
### ✨ 新增功能 (Added)
- **自動生成 AI 摘要並寄送**：大幅優化了 CronJob 的運作邏輯。現在系統在背景偵測到新影片時，會**直接在後端呼叫 Gemini API 產生重點摘要**，並將完整的分析結果直接包含在通知信件中寄給使用者。使用者不再需要點擊連結回到網站等待生成，體驗更加絲滑且主動。
- **後端摘要快取**：CronJob 生成的摘要會自動存入 Firebase 資料庫中。當使用者後續進入 Web 介面點擊該影片時，系統會直接讀取快取，秒速顯示結果，節省 API 呼叫成本與等待時間。
...
- **清除快取說明**：確認系統中已無寫死的舊信箱，該信箱僅存在於使用者的瀏覽器 `localStorage` 中。
...
### 💅 設定調整 (Changed)
- **通知信箱更新**：從 `server.ts`、`.env.example` 及 `gke/configmap.yaml` 中移除了舊信箱，目前系統僅會發送通知至主要信箱。
